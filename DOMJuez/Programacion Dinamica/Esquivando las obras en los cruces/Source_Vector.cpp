...
/*@ <answer>
    ...
    El coste de la función en espacio en el peor de los casos es O(M).
...
void evitandoObras(Matriz<char> ciudad, int N, int M) {
    ...
    vector<int> obras(M, 0);
    obras[0] = 1;

    for (int j = 1; j < M; j++) {
        if (ciudad[0][j] == 'X') obras[j] = 0;
        else obras[j] = obras[j - 1];
    }

    for (int i = 1; i < N; i++) {
        if (ciudad[i][0] == 'X') obras[0] = 0;
        for (int j = 1; j < M; j++) {
            if (ciudad[i][j] == 'X') obras[j] = 0;
            else obras[j] = obras[j - 1] + obras[j];
        }
    }
